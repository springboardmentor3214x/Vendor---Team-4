...
class ReportService:
    ...
    def get_procurement_report(
        ...
        # -----------------------------
        # Department-wise real aggregation
        # -----------------------------
        department_wise = {}
        for request in requests:
            department = request.department_name or 'Unspecified'
            row = department_wise.setdefault(department, {"requests": 0, "approved": 0, "orders": 0, "spending": 0})
            row["requests"] += 1
            if request.request_status == ProcurementStatus.APPROVED:
                row["approved"] += 1

        for po in purchase_orders:
            department = po.procurement_request.department_name if po.procurement_request and po.procurement_request.department_name else 'Unspecified'
            row = department_wise.setdefault(department, {"requests": 0, "approved": 0, "orders": 0, "spending": 0})
            row["orders"] += 1
            row["spending"] += float(po.total_cost or 0)
    ...
    def get_compliance_report(
        ...
        for item in compliance_data:
            ...
            if vendor_name:
                if vendor_name.strip().lower() not in vendor.company_name.lower():
                    continue
            ...
            if compliance_status:
                requested_status = compliance_status.strip().lower()
                actual_status = (
    ...
    def get_executive_summary(
        ...
        return {
            ...
            "metrics": [
                {"metric": "Total Registered Vendors", "value": total_vendors, "change": "—", "insight": "Current vendor count."},
                {"metric": "Active Vendors", "value": active_vendors, "change": "—", "insight": "Current active vendor count."},
                {"metric": "Procurement Spending", "value": procurement_spending, "change": "—", "insight": "Purchase-order expenditure in the selected period."},
                {"metric": "Procurement Completion Rate", "value": f"{procurement_completion_rate}%", "change": "—", "insight": "Completed procurement requests divided by total requests."},
                {"metric": "Delayed Deliveries", "value": delayed_deliveries, "change": "—", "insight": "Open purchase orders past expected delivery date."},
                {"metric": "Contracts Near Expiry", "value": contracts_near_expiry, "change": "—", "insight": "Contracts expiring within 60 days."},
                {"metric": "Average Compliance", "value": f"{average_compliance}%", "change": "—", "insight": "Average vendor compliance percentage."}
            ],
            ...
            "procurement_summary": {
                ...
                "completion_rate": procurement_completion_rate
            },

            "vendor_reliability_distribution": reliability_distribution,
            "top_performing_vendors": top_vendors,
            "delayed_deliveries": delayed_deliveries,
            "contracts_near_expiry": contracts_near_expiry,
            "average_compliance_percentage": average_compliance,
            "monthly_procurement_trends": monthly_trends
        }
