...
def get_certifications(db: Session):
    today = date.today()
    certifications = db.query(Certification).order_by(Certification.id.desc()).all()
    changed = False
    for certification in certifications:
        if certification.expiry_date < today and certification.status != CertificationStatus.EXPIRED:
