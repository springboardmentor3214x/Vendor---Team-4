...
def create_discussion(
    ...
    discussion, error = discussion_service.create_discussion(db, data)
    if error:
...
@router.get("/purchase-order/{purchase_order_id}", response_model=list[DiscussionResponse])
def get_purchase_order_discussions(
    purchase_order_id: int,
    db: Session = Depends(get_db)
):
    return discussion_service.get_purchase_order_discussions(db, purchase_order_id)
...
def get_discussion(
    ...
    discussion = discussion_service.get_discussion(db, discussion_id)
    if not discussion:
...
def update_discussion(
    ...
    discussion = discussion_service.update_discussion(db, discussion_id, data)
    if not discussion:
        raise HTTPException(status_code=404, detail="Discussion not found")
    return discussion
...
def delete_discussion(
    ...
    deleted = discussion_service.delete_discussion(db, discussion_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Discussion not found")
