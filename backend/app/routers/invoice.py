...
def update_invoice(
    ...
    return invoice
@router.patch("/{invoice_id}/verify", response_model=InvoiceResponse)
def verify_invoice(
    invoice_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
    ...
    if invoice.status == InvoiceStatus.REJECTED:
        raise HTTPException(status_code=400, detail="Rejected invoices cannot be verified.")
...
def reject_invoice(
    ...
    if invoice.status == InvoiceStatus.PAID:
        raise HTTPException(status_code=400, detail="A paid invoice cannot be rejected.")
...
async def upload_invoice(
    ...
    if suffix not in allowed:
        raise HTTPException(status_code=400, detail="Only PDF and image invoice files are supported.")
    upload_dir = Path("uploads") / "invoices"
    upload_dir.mkdir(parents=True, exist_ok=True)
    safe_name = f"{invoice.id}_{uuid.uuid4().hex}{suffix}"
    destination = upload_dir / safe_name
    content = await file.read()
    if len(content) > 10 * 1024 * 1024:
        raise HTTPException(status_code=400, detail="Invoice file must be 10 MB or smaller.")
    destination.write_bytes(content)
    invoice.attachment_name = file.filename
    invoice.attachment_path = str(destination)
    db.commit()
    db.refresh(invoice)
    return invoice
