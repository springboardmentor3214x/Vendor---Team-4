...
def get_my_vendor_profile(
    ...
    """
    Returns the vendor record linked to the logged-in account
    (matched by email, same convention as /me/dashboard).
    If none exists yet, a blank placeholder record is created
    automatically so the user always has their own profile to
    fill in -- they never see or touch anyone else's record.
    """
    ...
    vendor_number = last_vendor.id + 1 if last_vendor else 1
    vendor_id = f"VND{vendor_number:06d}"

    vendor = Vendor(
        vendor_id=vendor_id,
        company_name=f"Pending Vendor #{current_user.id}",
        vendor_category=VendorCategory.SERVICE,
        contact_person=current_user.full_name or "",
        designation="Vendor",
        email=current_user.email,
        phone=current_user.mobile_number or "0000000000",
        alternate_phone=None,
        gst_number=f"PENDING-{current_user.id}",
        pan_number=f"PENDING-{current_user.id}",
        company_registration_number=f"PENDING-{current_user.id}",
        address_line1="",
        address_line2=None,
        city="",
        state="",
...
def vendor_me_dashboard(
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user)
):
    vendor = db.query(Vendor).filter(Vendor.email == current_user.email).first()
    if not vendor:
        raise HTTPException(status_code=404, detail="No vendor profile is linked to the logged-in account.")
    ...
    performance = VendorPerformanceService.get_vendor_performance(db, vendor.id)
    reliability = VendorReliabilityService.get_vendor_reliability(db, vendor.id)
    orders = db.query(PurchaseOrder).filter(PurchaseOrder.vendor_id == vendor.id).all()
    active = sum(1 for o in orders if o.status not in (PurchaseOrderStatus.COMPLETED, PurchaseOrderStatus.CANCELLED))
    ...
    return {
        ...
        "vendor_status": vendor.vendor_status.value if vendor.vendor_status else None,
        ...
        "performance_score": performance.overall_vendor_score or 0,
        "active_purchase_orders": active,
        "completed_orders": completed,
        "on_time_delivery": performance.on_time_delivery_rate or 0,
...
def update_vendor(
    ...
    # a logged-in Vendor user may only edit their own
    # vendor record, never someone else's.
    # -------------------------
    if current_user.role.strip().lower() == "vendor" and vendor.email != current_user.email:
        raise HTTPException(
            status_code=403,
            detail="You are only allowed to update your own vendor profile"
        )
